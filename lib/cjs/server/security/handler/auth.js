...
let Auth = class Auth {
    async register(payload) {
        var errors = field_js_1.Field.validate(usr_js_1.default, payload);
    ...
    async login(payload) {
        if (!payload.lg) {
            return (0, index_js_2.Exception)({ ...index_js_3.HttpStatus[400], message: "Login is required" });
        }
        if (!payload.ps) {
            return (0, index_js_2.Exception)({ ...index_js_3.HttpStatus[400], message: "Password is required" });
        }
        var userRepository = connect_js_1.db.getRepository(usr_js_1.default);
    ...
    async refresh(payload) {
        ...
        var usr = await index_js_4.default.validate.refresh(payload);
    ...
    async logout(payload) {
        ...
        var sessionRepository = connect_js_1.db.getRepository(se_js_1.default);
        await sessionRepository.update({ rt: payload }, {
            ac: false,
            lou: new Date(),
        });
    }
    async me(payload) {
        ...
        var usr = await index_js_4.default.validate.access(payload);
    ...
    async changePassword(payload) {
        var errors = field_js_1.Field.validate(usr_js_1.default, { ps: payload.ps });
        if (Object.keys(errors).length > 0) {
            return (0, index_js_2.Exception)({
                ...index_js_3.HttpStatus[400],
                message: errors,
        ...
        var ticket = await index_js_4.default.validate.recover(payload.tk);
    ...
    async forgotPassword(payload) {
        ...
        var { token } = await index_js_4.default.forget(payload);
        var changePasswordRepository = connect_js_1.db.getRepository(crp_js_1.default);
