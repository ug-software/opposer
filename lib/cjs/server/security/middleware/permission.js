...
exports.default = async (req, res, next) => {
    ...
    if (request.model) {
        const allModels = system.getAllModels();
        const model = (await allModels).find((x) => x.name === request.model);
        if (model) {
            const isPublic = (0, index_js_4.getIsPublicMetadata)(model.entity);
            if (isPublic) {
                return next();
            }
        }
