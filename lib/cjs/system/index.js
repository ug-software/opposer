...
async function getAllModels() {
    ...
    let modelsPath = path_1.default.resolve(root, "src", "models");
    if (settings.models) {
        modelsPath = path_1.default.resolve(root, settings.models, "models");
    }
    const schemaFiles = fs_1.default.readdirSync(modelsPath);
    const allModels = await Promise.all(schemaFiles.map(async (schemaPathName) => {
        const name = getFileName(schemaPathName, false);
        const filePath = path_1.default.resolve(modelsPath, schemaPathName);
