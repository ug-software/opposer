...
exports.default = async (req, res) => {
    ...
    if (props.handler) {
        ...
        if (Array.isArray(allDto) && allDto.length > 0) {
            var payloadMetadata = allDto.find((x) => x.name === helper.toCamelCase(method));
            if (payloadMetadata) {
                var erros = field_js_1.Field.validate(payloadMetadata.dto, payload);
        ...
        var resultHandler = await new __meta.handler()[helper.toCamelCase(method)](payload);
    ...
    var erros = field_js_1.Field.validate(model, props);
    if (Object.keys(erros).length > 0) {
        res.status(400).json({
            erros,
            message: "Data not valid for method, verify erros and try again.",
