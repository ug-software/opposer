...
async function initializeDatabase(props, modelsPath) {
    const settings = index_js_2.default.getSettingsFile();
    const allModels = await index_js_2.default.getAllModels();
...
async function ensureManager(db, settings, modelsPath) {
    if (!settings.auth)
        return;
    try {
        const allModels = await index_js_2.default.getAllModels();
...
async function Server(props) {
    ...
    index_js_5.default.setContext("modelsPath", props.modelsPath);
    index_js_5.default.setContext("handlersPath", props.handlersPath);
    console.log("-> Initializing scheduler.");
    await index_js_3.default.initialize(props.schedulesPath);
